...
async function sendNtfy(cfg, msg) {
...
async function sendDiscord(cfg, msg) {
...
async function sendGeneric(cfg, msg, payload) {
  ...
  if (cfg.secret) headers['x-tkb-signature'] = crypto.createHmac('sha256', cfg.secret).update(body).digest('hex');
 ...
 */
async function dispatch(webhooks, msg, payload = {}, opts = {}) {
  const results = [];
...
module.exports = { dispatch, sendNtfy, sendDiscord, sendTelegram, sendGeneric, chunk };
