...
function renderHeader() {
  ...
  loginBtn.textContent = s.authState === 'ok' ? 'Re-sign in' : 'Sign in';
  ...
  if (s.authState === 'expired') {
    banner.hidden = false;
    banner.textContent = 'Your portal session has expired. Background checks are paused until you sign in again.';
...
function renderWeek() {
  ...
  $('week-label').textContent = `${fmtDay(weekStart)} – ${fmtDay(end)} ${end.getFullYear()} · ${weekSessions} session${weekSessions === 1 ? '' : 's'}`;
...
function renderView() {
  ...
  document.querySelector('.nav').style.visibility = view === 'week' ? 'visible' : 'hidden';
...
function bind() {
  ...
  $('btn-prev').addEventListener('click', () => { weekStart = addDays(weekStart, -7); renderWeek(); });
  $('btn-next').addEventListener('click', () => { weekStart = addDays(weekStart, 7); renderWeek(); });
  $('btn-today').addEventListener('click', () => { weekStart = startOfWeek(new Date()); renderWeek(); });
  document.querySelectorAll('.view-switch .seg').forEach((b) => b.addEventListener('click', () => { view = b.dataset.view; renderView(); }));
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (view === 'week' && !e.target.closest('input,select')) {
      if (e.key === 'ArrowLeft') { weekStart = addDays(weekStart, -7); renderWeek(); }
      if (e.key === 'ArrowRight') { weekStart = addDays(weekStart, 7); renderWeek(); }
    }
  });
  ...
  setInterval(() => { if (state) renderHeader(); }, 30000);
}
